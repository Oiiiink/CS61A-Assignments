...
def balanced(m):
    ...
    """
    def torque(s):
        return length(s) * total_mass(end(s))
    if is_planet(m) or (torque(left(m)) == torque(right(m)) and balanced(end(left(m))) and balanced(end(right(m)))):
        return True 
    else :
        return False
